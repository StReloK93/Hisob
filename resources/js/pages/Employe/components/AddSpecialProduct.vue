<template>
    <v-dialog v-model="pageData.dialog" scrollable width="600" location="right">
        ...
        <v-form ref="form" @submit.prevent="addProductEmploye" @vue:mounted="formMount">
...
function formMount() {
    axios.get(`employe-get-position-products/${employe.id}`).then(({ data }) => {
        const array = data.map((item) => { return { ...item, name: item.product?.name, count: 1, nomenclature: null, price: null, productPrices: [] } })
        pageData.products = array
    })
}
